...
search_result = vs_mcp_client.call_tool(
    tool_name, {"query": "financial inclusion mobile banking"}
)
...
if vector_search_tool_name in tools_dict:
    ...
    result = search_tool.exec_fn(query="account ownership gender gap")
...
response = agent.chat(
    "What do the Global Findex reports say about unbanked adults in Sub-Saharan Africa?"
)
