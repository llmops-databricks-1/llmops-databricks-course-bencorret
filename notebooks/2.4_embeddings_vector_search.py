...
logger.info(
    f"  Source: {vs_manager.catalog}.{vs_manager.schema}.global_findex_chunks_table"
)
logger.info(f"  Embedding Model: {vs_manager.embedding_model}")
...
results = index.similarity_search(
    query_text=query, columns=["text", "unique_id", "title", "id"], num_results=5
)
...
logger.info(f"Query: {query}")
logger.info("Filter: title = Global findex 2025 - Executive report\n")
logger.info("Results:")
logger.info("=" * 80)
